...
// Observed rainfall accumulation [0, windowH] in mm from GPM IMERG daily grids.
// Daily product: windows must be whole days (24/48/72/168h).
export function xrObsTileUrl(date: string, windowH: number): string {
  if (windowH % 24 !== 0) {
    throw new Error(`observation needs a daily-multiple window: ${windowH}h`);
  }
  const qs = new URLSearchParams({
    date,
    layer: 'obs',

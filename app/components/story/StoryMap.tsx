...
function legendFor(ch: ChapterConfig | null): LegendSpec | null {
  ...
  if (ch.layers.includes('rainfall')) {
    return {
      ...
      gradient: { css: 'linear-gradient(90deg,#deebf7,#6baed6,#08306b)', min: '1 mm', max: '100 mm+' },
    };
  }
  ...
  if (ch.layers.includes('emdat')) {
    return { title: 'EM-DAT flood match', classes: [{ color: '#7c3aed', label: 'Matched regions' }] };
  }
  return null;
}
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      map.on('moveend', () => {
        ...
        setStateText(`center ${c.lng.toFixed(1)},${c.lat.toFixed(1)} · zoom ${map.getZoom().toFixed(1)} · ${date}`);
      });
      ...
      map.on('sourcedataloading', (e) => {
        if (e.sourceId === 'exceedance' || e.sourceId === 'rainfall') setTilesLoading(true);
      });
    ...
    <div className='map' ref={containerRef}>
      ...
      {tilesLoading ? (
        <div className='map-loading'>
          <i className='spin' /> Rendering forecast field
        </div>
      ) : null}
      {userMoved ? (
        <button
